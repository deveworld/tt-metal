...
// SHARED IMPLEMENTATION - works with vector of output tensors (exposed for ternary_matmul_split)
TernaryMatmulProgramFactory::shared_variables_t ternary_matmul_factory_helper_common(
    ...
    auto in0_sender_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ternary_matmul/device/kernels/dm_in0_sender.cpp",
        in0_sender_cores,
    ...
    auto in1_sender_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ternary_matmul/device/kernels/dm_in1_sender_out.cpp",
        in1_sender_cores,
        tt::tt_metal::DataMovementConfig{
    ...
    auto compute_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ternary_matmul/device/kernels/compute.cpp",
        core_grid,
    ...
    return TernaryMatmulProgramFactory::shared_variables_t{
        num_cores,
        cores,
...
TernaryMatmulProgramFactory::shared_variables_t ternary_matmul_factory_helper(
    tt::tt_metal::Program& program,
    const Tensor& input_tensor,
    const Tensor& weight_tensor,
    const std::optional<const Tensor>& bias_tensor,
    ...
    return ternary_matmul_factory_helper_common(
        ...
        fused_op_signaler,
        1,  // N_chunks = 1 for regular ternary_matmul
        std::nullopt,
        std::nullopt,
        std::nullopt,
...
TernaryMatmulProgramFactory::cached_program_t TernaryMatmulProgramFactory::create(
    const TernaryMatmulParams& operation_attributes,
...
void TernaryMatmulProgramFactory::override_runtime_arguments(
