...
TernaryMatmulSimpleProgramFactory::cached_program_t TernaryMatmulSimpleProgramFactory::create(
    ...
    // Small in0_block_w lets reader/writer prefetch the next K slice while
    // compute consumes the current one (reader/writer/compute pipelining
    // across K iterations). Empirically 8 is a sweet spot.
